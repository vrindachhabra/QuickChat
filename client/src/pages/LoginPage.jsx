      ...
      <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
        ...
        )}
        {
          currState === "Sign up" && isDataSubmitted && (
            ...
            rows={4} className='p-2 border border-gray-500 rounded-md focus:outline-none' required placeholder='Provide a short bio..'></textarea>
          )
        }
        ...
        <div className='flex flex-col gap-2'>
          {currState === "Sign up" ? (
            <p className='text-sm text-gray-600'>Already have an account? <span className='font-medium text-violet-400 cursor-pointer' onClick={() => {setCurrentState("Login");  setIsDataSubmitted(false)}}>Login</span></p>
          ) : (
            <p className='text-sm text-gray-600'>Create an account <span onClick={() => {setCurrentState("Sign up");  setIsDataSubmitted(false)}} className='font-medium text-violet-400 cursor-pointer'>Click here</span></p>
          )}
        </div>
      </form>
    </div>
